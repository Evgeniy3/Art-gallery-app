...
import { authorsSelect, fetchAuthors } from '../../store/authorSlice';
import { useAppDispatch } from '../../store/store';
import { Authors } from '../../store/authorSlice/types';
import clearSvg from '../../assets/img/clear.svg';
import { setAuthorId } from '../../store/filterSlice';

const SearchAuthor: React.FC = React.memo(() => {
  ...
  const onChangeSelect = (authorValue: React.SetStateAction<string>) => {
    setValue(authorValue);
    const authorId = authors.find((author: Authors) => (author.name === authorValue ? author.id : '')) as Authors;

    dispatch(
      setAuthorId(String(authorId.id)),
    );
  };
  ...
  return (
    <div className="dropdown">
      ...
      <ul onClick={() => setActive(!active)} className="dropdown_list">
        {authors.map((author: Authors) => <li onClick={(e) => onChangeSelect((e.target as HTMLElement).innerText)} className="dropdown_list-item" key={author.id}>{author.name}</li>)}
      </ul>
      )}
      {value
        && (
